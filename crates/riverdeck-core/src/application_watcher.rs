...
pub static APPLICATION_PROFILES: Lazy<RwLock<Store<ApplicationProfiles>>> = Lazy::new(|| {
    RwLock::new(Store::new("applications", &crate::shared::config_dir(), HashMap::new()).unwrap())
});

pub static APPLICATION_PROCESSES: Lazy<RwLock<HashMap<String, Vec<u32>>>> =
    Lazy::new(|| RwLock::new(HashMap::new()));
pub static APPLICATION_PLUGINS: Lazy<RwLock<HashMap<String, Vec<String>>>> =
    Lazy::new(|| RwLock::new(HashMap::new()));
    ...
    tokio::spawn(async move {
        ...
        loop {
            let app_name = if let Ok(win) = get_active_window() {
                ...
                if !applications.contains(&win.app_name)
                    && !win
                        .app_name
                        .to_lowercase()
                        .starts_with(&crate::shared::PRODUCT_NAME.to_lowercase())
                    && !win.app_name.trim().is_empty()
                {
                    applications.push(win.app_name.clone());
                    ui::emit(UiEvent::ApplicationsUpdated {
            ...
            if app_name != previous {
                ...
                for value in crate::shared::DEVICES.iter() {
                    ...
                    let Some(profile) = application
                        ...
                        .or(default.and_then(|d| d.get(device)))
                    else {
                        continue;
                    };
                    if crate::store::profiles::DEVICE_STORES
                        .write()
                        .await
        ...
        loop {
            ...
            for (application, plugins) in application_plugins.iter() {
                for process in system.processes_by_exact_name(application.as_ref()) {
                    ...
                    let pids = application_processes
                        .entry(application.clone())
                        .or_default();
                    if !pids.contains(&pid) {
                        pids.push(pid);
                        for plugin in plugins {
                            let _ = crate::events::outbound::applications::application_did_launch(
                                plugin,
                                application.clone(),
                            )
                            .await;
...
pub async fn start_monitoring(plugin: &str, applications: &Vec<String>) {
    ...
    for application in applications {
        application_plugins
            .entry(application.to_owned())
            .or_default()
            .push(plugin.to_owned());
