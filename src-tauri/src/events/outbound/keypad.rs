...
pub async fn key_down(device: &str, key: u8) -> Result<(), anyhow::Error> {
	...
	if matches!(instance.action.uuid.as_str(), "riverdeck.multiaction" | "opendeck.multiaction") {
	...
	} else if matches!(instance.action.uuid.as_str(), "riverdeck.toggleaction" | "opendeck.toggleaction") {
...
pub async fn key_up(device: &str, key: u8) -> Result<(), anyhow::Error> {
	...
	} else if !matches!(instance.action.uuid.as_str(), "riverdeck.multiaction" | "opendeck.multiaction") {
